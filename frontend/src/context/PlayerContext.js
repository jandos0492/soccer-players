...
export const PlayerProvider = ({ children }) => {
  ...
  useEffect(() => {
    fetch("http://localhost:8081/proxy/api/")
  ...
  const deletePlayer = (id) => {
    return new Promise((resolve, reject) => {
      fetch(`http://localhost:8081/proxy/api/${id}`, {
        method: "DELETE",
      })
        .then((response) => {
          if (response.ok) {
  ...
  const updatePlayer = (id, updatedData) => {
    return fetch(`http://localhost:8081/proxy/api/${id}/edit`, {
      method: "PUT",
      headers: {
  ...
  const createPlayer = async (playerData) => {
    const response = await fetch("http://localhost:8081/proxy/api/", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
